...
CDirect3D* CDirect3D::s_direct3d = NULL;
...
bool CDirect3D::Initialize(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, float screenDepth, float screenNear)
{
	...
	if (FAILED(m_device->CreateBlendState(&blendStateDesc, &m_blendState))) {
		return false;
	}
...
void CDirect3D::GetViewMatrix(D3DMATRIX& viewMatrix)
{
	viewMatrix = m_viewMatrix;
	return;
}

void CDirect3D::SetViewMatrix(D3DMATRIX viewMatrix)
{
...
HWND CDirect3D::GetHwnd()
{
	return m_hwnd;
}

void CDirect3D::GetScreenSize(int& screenWidth, int& screenHeight)
{
