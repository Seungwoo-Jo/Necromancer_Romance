...
#include "CMonster_Shoa.h"

CMonster_Shoa::CMonster_Shoa()
{
}
...
int CMonster_Shoa::skill_1()
{
	if(getMyTurn())
	{
		...
		m_bTargeting = false;
		float damage = (temp.m_atk - targetCard->getStat().m_def) * 4.5f;
		if(damage < 0) {
			damage = 0;
		}
...
void CMonster_Shoa::Update()
{

}

int CMonster_Shoa::UseSkill(int num)
{
	if(num == 0) {
		return skill_1();
	}
