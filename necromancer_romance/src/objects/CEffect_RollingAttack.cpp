...
CEffect_RollingAttack::CEffect_RollingAttack()
{
...
CEffect_RollingAttack::CEffect_RollingAttack(const CEffect_RollingAttack& eft)
{
}
...
void CEffect_RollingAttack::Initialize()
{
	m_effectPos.setPosition(0, 0, 0);
	m_sustainmentTime = 1200;
	m_attackNum = 1;
	m_attackTiming = 0.5f;
	m_damage = 0;

	m_effectSprite = new CBitmap();
	m_effectSprite->Initialize(L"res/effect/dummy_rolling_attack.png", 0.0f, 0.0f, 1.0f, 1.0f, 1.0f, 1.0f);
	m_effectSprite->SetSpriteAnimation(4, 4, m_sustainmentTime, false);
...
void CEffect_RollingAttack::Shutdown()
{
...
void CEffect_RollingAttack::SetDamage(int num, int totalDamage)
{
	m_attackNum = num;
	m_damage = static_cast<int>(totalDamage/num);
}

bool CEffect_RollingAttack::GetAlive()
{
	return !m_effectTimer->GetTimer();
}

void CEffect_RollingAttack::Update()
{
...
void CEffect_RollingAttack::Render()
{
	m_effectSprite->Render(448, 100, m_effectPos.posZ, 1.5f, 1.5f);
