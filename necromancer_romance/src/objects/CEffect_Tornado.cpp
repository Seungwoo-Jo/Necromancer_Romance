...
CEffect_Tornado::CEffect_Tornado()
{
...
CEffect_Tornado::CEffect_Tornado(const CEffect_Tornado& eft)
{
}
...
void CEffect_Tornado::Initialize()
{
	m_effectPos.setPosition(0, 0, 0);
	m_sustainmentTime = 500;
	m_attackNum = 1;
	m_attackTiming = 0.125f;
	m_damage = 0;

	m_effectSprite = new CBitmap();
	m_effectSprite->Initialize(L"res/effect/dummy_tornado.png", 0.0f, 0.0f, 1.0f, 1.0f, 1.0f, 1.0f);
	m_effectSprite->SetSpriteAnimation(2, 2, m_sustainmentTime, false);
...
void CEffect_Tornado::SetDamage(int num, int totalDamage)
{
	m_attackNum = num;
	m_damage = static_cast<int>(totalDamage/num);
}

bool CEffect_Tornado::GetAlive()
{
	return !m_effectTimer->GetTimer();
}

void CEffect_Tornado::Update()
{
