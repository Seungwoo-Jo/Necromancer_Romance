...
CEffect_IceArrow::CEffect_IceArrow()
{
...
CEffect_IceArrow::CEffect_IceArrow(const CEffect_IceArrow& eft)
{
}
...
void CEffect_IceArrow::Initialize()
{
	m_effectPos.setPosition(0, 0, 0);
	m_sustainmentTime = 550;
	m_attackNum = 1;
	m_attackTiming = 0.12f;
	m_damage = 0;

	m_effectSprite = new CBitmap();
	m_effectSprite->Initialize(L"res/effect/eadirc_ice_arrow.png", 0.0f, 0.0f, 1.0f, 1.0f, 1.0f, 1.0f);
	m_effectSprite->SetSpriteAnimation(4, 2, m_sustainmentTime, false);
	...
	m_sound = new CSound();
	m_sound->Initialize("res/sound/se/icearrow.wav");
	m_sound->Loop(false);
}

void CEffect_IceArrow::Shutdown()
{
...
void CEffect_IceArrow::SetDamage(int num, int totalDamage)
{
	m_attackNum = num;
	m_damage = static_cast<int>(totalDamage/num);
}

bool CEffect_IceArrow::GetAlive()
{
	return !m_effectTimer->GetTimer();
}

void CEffect_IceArrow::Update()
{
